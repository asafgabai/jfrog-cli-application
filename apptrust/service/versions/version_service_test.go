...
func TestReleaseAppVersion(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			_, err := service.ReleaseAppVersion(mockCtx, tt.applicationKey, tt.version, tt.payload, tt.sync)
			if tt.expectedError == "" {
				assert.NoError(t, err)
			} else {
				assert.Error(t, err)
...
func TestUpdateAppVersion(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			_, err := service.UpdateAppVersion(mockCtx, "test-app", "1.0.0", tt.request)
			if tt.expectError {
				assert.Error(t, err)
				if tt.errorMsg != "" {
